...
function hslToRgb(hsl) {
	const h = Number(hsl.split(',')[0].replace('hsl(', ''));
	const s = Number(hsl.split(',')[1].replace('%', '')) / 100;
	const l = Number(hsl.split(',')[2].replace('%)', '')) / 100;
...
writeFileSync('./colors1.txt', txtColors1);
...
for (let i = 0; i < 10; i += 1) {
	colors2.push(selectColor(Math.random()));
}

let txtColors2 = '';
colors2.forEach((d) => {
	txtColors2 += `${hslToRgb(d)}\n`;
});

writeFileSync('./colors2.txt', txtColors2);